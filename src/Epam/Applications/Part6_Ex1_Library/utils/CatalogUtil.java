...
import Epam.Applications.Part6_Ex1_Library.entities.*;
...
public class CatalogUtil {
    ...
    public static boolean printBooks(List<Book> books) {
        if (books.size() != 0) {
            ...
            while (page > 0 && page <= numberOfPages) {
                ...
                System.out.print("Enter page number or 0 to back to the menu: ");
    ...
    public static boolean addBook(Catalog catalog, Book book) {
        if (!catalog.getBooks().contains(book)) {
            ...
            FileManager.saveCatalog(catalog.getBooks());
    ...
    public static boolean suggestBook(Catalog catalog, Book book) {
        if (!catalog.getBooks().contains(book)) {
            Letter letter = new Letter(UserUtil.getAdmin().getEmail(), catalog.getUser().getEmail(), book);
    ...
    public static boolean findBook(Catalog catalog) {
        ...
        int request = InputUtil.checkInputChoice(EUserType.USER);
        if (request == 1) {
