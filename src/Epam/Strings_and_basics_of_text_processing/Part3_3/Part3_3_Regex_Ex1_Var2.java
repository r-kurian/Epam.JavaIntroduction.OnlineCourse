...
public class Part3_3_Regex_Ex1_Var2 {
    ...
    public static String[] paragraphSort(String[] paragraphArray) {
    ...
    public static void main(String[] args) throws IOException {
        ...
        System.out.println("Enter your file path:"); // C:\Users\UserName\Desktop\java.txt
        ...
        String myStringFromFile = scannerForFile.useDelimiter("\\A").next();
        scannerForFile.close();

        String[] paragraphArray = paragraphSplit(myStringFromFile);
        String paragraphString = paragraphArray.toString();
        String[] paragraphSort = sentenceSplit(paragraphString);
        String[] paragraphSortResult = paragraphSort(paragraphArray);
        String[] sentence = sentenceSplit(paragraphArray[1]);


        Scanner scannerForPrinting = new Scanner(System.in);

        System.out.println("\n >>> Enter 1 to PRINT all PARAGRAPHS. Enter 0 to skip: ");
        int printParagraphsArray = scannerForPrinting.nextInt();
        if (printParagraphsArray == 1) {
            System.out.println(Arrays.toString(paragraphArray));
        }

        System.out.println("\n >>> Enter 1 to SORT all PARAGRAPHS. Enter 0 to skip: ");
        int printSortedParagraphs = scannerForPrinting.nextInt();
        if (printSortedParagraphs == 1) {
            System.out.println(Arrays.toString(paragraphSortResult));
        }

        System.out.println("\n >>> Enter 1 to SORT all SENTENCE in first paragraph. Enter 0 to skip: ");
        int printSortedSentence = scannerForPrinting.nextInt();
        if (printSortedSentence == 1) {
        ...
        if (printETWAS == 1) {
            System.out.println(wordSort(paragraphSortResult));
        }
    }
}